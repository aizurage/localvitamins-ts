...
export const EventRecruiterPanel: FC<{eventRecruiter: EventRecruiter}> = ({eventRecruiter}) => {
    ...
    useEffect(() => {
        (async() => {
            const eventRecruiterPictureObjectUrl = await downloadRecruiterImage(eventRecruiter.imageUrl)
    ...
    return(
        <div className={styles.eventRecruiterPanelStyle}>
            ...
            <HeadingDescriptionBlock heading="自己紹介" description={eventRecruiter.introduction}/>
            <HeadingDescriptionBlock heading="一言コメント" description={eventRecruiter.comment}/>
        </div>
    )
}